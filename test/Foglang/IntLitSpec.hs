...
spec = do
  ...
  describe "intLit" $ do
    it "parses go spec examples" $
      mapM_ (\(s, con) -> parseIntLit s `shouldBe` Right (con s)) goSpecExamples

    it "parses extra examples" $
      mapM_ (\(s, con) -> parseIntLit s `shouldBe` Right (con s)) extraExamples

    it "rejects go spec invalid examples" $
      mapM_ (\s -> parseIntLit s `shouldSatisfy` isLeft) goSpecInvalidExamples
