...
```fog
let printf (format : string) (args : ...any) => unit =
...
```fog
let run (cmd : string) (args : ...string) => unit = ...

run                  -- reference to function value
run "ls"             -- partial application: ...string => unit
...
```fog
let myPrintf (format : string) (args : ...any) => unit =
